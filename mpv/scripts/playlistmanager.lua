...
--END OF SAMPLE REPLACE ]=====]
  ...
  -- ratio of Chinese font width and English font width characters
  chinese_font_width = 2
}
...
function len_char(s, cnCharLen)
  ...
  for i=1,#sTb do
      local utfCharLen = string.len(sTb[i])
      if utfCharLen > 1 then
          tempLen = cnCharLen
      else
          tempLen = 1
      end
      charLen = charLen + tempLen
  end

  return charLen
end

function get_len_limit(s, charLimit, cnCharLen)
  local sTb = str_tb(s)
  local utfLen = 0
  local tempLen = 0
  local count = charLimit
...
function stripfilename(pathfile, media_title)
  ...
  if settings.slice_longfilenames and len_char(tmp, settings.chinese_font_width)>settings.slice_longfilenames_amount+5 then
    tmp = tmp:sub(1, get_len_limit(tmp, settings.slice_longfilenames_amount, settings.chinese_font_width)).." ..."
  end
  return tmp
end
